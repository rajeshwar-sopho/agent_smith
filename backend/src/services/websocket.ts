...
class WebSocketManager {
  ...
  log(botId: string, taskId: string | null, message: string, level = 'info', meta?: unknown, runNumber = 1) {
    this.emitToBot(botId, {
      type: 'log',
      botId,
      payload: { taskId, message, level, meta, runNumber, ts: new Date().toISOString() },
    });
  }
